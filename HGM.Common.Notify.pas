...
begin
  if TNotifyControl(Panel).MouseInClient or PtInRect(Panel.ClientRect, Panel.ScreenToClient(Mouse.CursorPos)) then
    Exit;
  HideNotify;
end;
...
begin
  if not Assigned(FOwner) then
    raise Exception.Create('Необходимо указать форму владельца (OwnerForm)');
  FLabelCaption.Caption := FLastNotify.Caption;
  FLabelText.Caption := FLastNotify.Text;
  FPanelLeft.Color := FLastNotify.Color;
