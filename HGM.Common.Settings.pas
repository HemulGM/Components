...
begin
 case Value of
  rrHKLM: Result:=HKEY_LOCAL_MACHINE;
  rrHKCU: Result:=HKEY_CURRENT_USER;
 ...
 with CreateReg(KEY_READ, RegRootToHKEY(FRoot)) do
  begin
   try
    ...
    if Result then Result:=ValueExists(Param);
   except
    Result:=False;
   end;
   Free;
...
begin
 ...
 with CreateReg(KEY_READ, RegRootToHKEY(FRoot)) do
  begin
   try
    Result:=OpenKeyReadOnly(Path+'\'+Section);
    if Result then Value:=ReadBool(Param);
   except
    Result:=False;
   end;
   Free;
...
begin
 ...
 with CreateReg(KEY_READ, RegRootToHKEY(FRoot)) do
  begin
   try
    Result:=OpenKeyReadOnly(Path+'\'+Section);
    if Result then Value:=ReadDateTime(Param);
   except
    Result:=False;
   end;
   Free;
...
begin
 ...
 with CreateReg(KEY_READ, RegRootToHKEY(FRoot)) do
  begin
   try
    Result:=OpenKeyReadOnly(Path+'\'+Section);
    if Result then Value:=ReadFloat(Param);
   except
    Result:=False;
   end;
   Free;
...
begin
 ...
 with CreateReg(KEY_READ, RegRootToHKEY(FRoot)) do
  begin
   try
    Result:=OpenKeyReadOnly(Path+'\'+Section);
    if Result then Value:=ReadInteger(Param);
   except
    Result:=False;
   end;
   Free;
...
begin
 ...
 with CreateReg(KEY_READ, RegRootToHKEY(FRoot)) do
  begin
   try
    Result:=OpenKeyReadOnly(Path+'\'+Section);
    if Result then Value:=ReadString(Param);
   except
    Result:=False;
   end;
   Free;
 ...
 with CreateReg(KEY_WRITE, RegRootToHKEY(FRoot)) do
  begin
   try
    ...
    if Result then WriteBool(Param, Value);
   except
    Result:=False;
   end;
   Free;
...
begin
 ...
 with CreateReg(KEY_WRITE, RegRootToHKEY(FRoot)) do
  begin
   try
    Result:=OpenKey(Path+'\'+Section, True);
    if Result then WriteDateTime(Param, Value);
   except
    Result:=False;
   end;
   Free;
...
begin
 ...
 with CreateReg(KEY_WRITE, RegRootToHKEY(FRoot)) do
  begin
   try
    Result:=OpenKey(Path+'\'+Section, True);
    if Result then WriteFloat(Param, Value);
   except
    Result:=False;
   end;
   Free;
...
begin
 ...
 with CreateReg(KEY_WRITE, RegRootToHKEY(FRoot)) do
  begin
   try
    Result:=OpenKey(Path+'\'+Section, True);
    if Result then WriteInteger(Param, Value);
   except
    Result:=False;
   end;
   Free;
...
begin
 ...
 with CreateReg(KEY_WRITE, RegRootToHKEY(FRoot)) do
  begin
   try
    Result:=OpenKey(Path+'\'+Section, True);
    if Result then WriteString(Param, Value);
   except
    Result:=False;
   end;
   Free;
 ...
 with CreateIni do
  begin
   try
    ...
    if Result then Result:=ValueExists(Section, Param);
   except
    Result:=False;
   end;
   Free;
...
begin
 ...
 with CreateIni do
  begin
   try
    Result:=FileExists(FileName);
    if Result then Value:=ReadBool(Section, Param, Default);
   except
    Result:=False;
   end;
   Free;
...
begin
 ...
 with CreateIni do
  begin
   try
    Result:=FileExists(FileName);
    if Result then Value:=ReadDateTime(Section, Param, Default);
   except
    Result:=False;
   end;
   Free;
...
begin
 ...
 with CreateIni do
  begin
   try
    Result:=FileExists(FileName);
    if Result then Value:=ReadFloat(Section, Param, Default);
   except
    Result:=False;
   end;
   Free;
...
begin
 ...
 with CreateIni do
  begin
   try
    Result:=FileExists(FileName);
    if Result then Value:=ReadInteger(Section, Param, Default);
   except
    Result:=False;
   end;
   Free;
...
begin
 ...
 with CreateIni do
  begin
   try
    Result:=FileExists(FileName);
    if Result then Value:=ReadString(Section, Param, Default);
   except
    Result:=False;
   end;
   Free;
